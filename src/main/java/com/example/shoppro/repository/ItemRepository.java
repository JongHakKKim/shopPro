...
public interface ItemRepository extends JpaRepository<Item, Long> , ItemsearchRepository {
    ...
    public List<Item> selectwhereItem(String itemNm);

    @Query("select i from Item i where i.itemNm like concat('%', :itemNm,'%')")
    public List<Item> findByItemContaining(String itemNm);
    ...
    //nativeQuery 사용
    @Query(value = "select * from Item i where i.item_nm = :a", nativeQuery = true)
    List<Item> nativeQuerySelectwhereNamelike(@Param("a") String itemNm, Pageable pageable);
}
